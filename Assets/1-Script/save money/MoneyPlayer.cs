...
public class MoneyPlayer : MonoBehaviour
{
    ...
    public void ArmorMoney()
    {
        if (armorLevel == 0)
        {
            PlayerPrefs.SetInt("Player Money", playerMoney - 50);
        }
        else if (armorLevel == 2)
        {
            PlayerPrefs.SetInt("Player Money", playerMoney - 80);
        }
    }
    void Update()
    {
        playerMoney = PlayerPrefs.GetInt("Player Money");
        armorLevel = PlayerPrefs.GetInt("level_int");
        Coin.text = playerMoney + "";
    }
}
